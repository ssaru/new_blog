---
title: (MML Book 선형대수 Chapter ~2.2) 선형대수/벡터/선형방정식/매트릭스
...
선형대수가 어떤 학문인지, 벡터, 선형방정식, 매트릭스의 정의는 무엇이고, 어떤 속성을 갖는지에 대해서 살펴봅니다.
...
2. **Polynomial vectors:**
   ...
   Figure 2.1 (b)에서 표현하고있는 polynomial(다항식) 또한 벡터입니다.
...
수학에서 하나의 중요한 개념는 **닫혀있다(closure)**라는 개념입니다. 

만약 내가 새로운 연산자(operator)를 만들었다고 생각해봅시다. 이 때, **내가 제안한 연산(operation)으로 만들어지는 것들의 집합은 무엇인가?**라는 궁금증이 생길 수 있습니다.

이를 선형대수 관점에서 다시 생각해보면, **작은 벡터의 집합에서 시작해서 서로 더해지고, 스케일링되었을때의 그 결과인 벡터들의 집합은 무엇인가?**와 같습니다. 이 질문에 대한 답은 Section 2.4에서 언급할 vector space가 됩니다. vector space의 개념과 속성은 머신러닝 알고리즘의 기초가 됩니다.
...
## 선형 방정식(Systems of Linear Equations)

선형 방정식은 선형 대수에서 중심적인 역활을 합니다. 많은 문제들은 선형 방정식으로 형식화되며, 선형대수는 이를 풀 수 있는 도구를 제공합니다.



**Example 2.1**

회사에서 제품 $$N_{1}, ... ,N_{n}$$ 을 생산한다고합니다. 이 때, 이 제품들을 생산하기 위한 리소스는 $$R_{1}, ... ,R_{N}$$ 가 요구됩니다. 더 자세히 이야기하면, 제품 $$N_{j}$$를 생산하기 위해서, 리소스 $$R_{i}$$의 단위인 $$a_{ij}$$가 필요합니다. ($$i=1,...,m$$, $$j=1,...,n$$)

이 때, 우리의 목적은 한정된 리소스에서 얼마나 많은 제품을 만들 수 있는지에 대한 최적의 생산 계획을 찾는 것입니다. 우리가 제품 특정 제품 $$x_{i},...,x_{n}$$ 가 있을 때, 이를 생산하기 위해서 리소스 $R_{i}​이 필요합니다. 이를 수학적으로 나타내면 아래와 같습니다.

$$a_{i1}x_{1} + ... + a_{in}x_{n}\ \  (2.2)$$



이 때, $$(x_{1},...,x_{n}) \in R^{n}$$ 이므로 최적의 생산 계획은 아래의 선형 시스템을 만족하는 해를 찾으면 됩니다.
...
식 (2.3)은 일반적인 형태의 선형 방정식입니다.

$$x_{1},...,x_{n},\ \ \ \ (x_{1},...,x_{n}) \in R^{n}$$ 

은 시스템에서 미지수가 되며, 식 (2.3)을 만족하는 해가 이 선형 방정식의 해가 됩니다.
...
이 선형 방정식은 해가 없습니다. (1)과 (2)를 더하면 $$2x_{1} + 3x_{3} = 5$$입니다. 이 결과를 (3)과 빼게되면, 부등식은 모순이 됩니다. 이 경우에는 선형 방정식에 해는 없게됩니다.
...
이를 똑같이 풀어보면, (1)과 (2)를 더한 결과가 (3)과 같음을 할 수 있습니다.
...
일반적으로 실수값을 갖는 선형 방정식으로 구성된 시스템에서 우리는 아래와 같은 종류의 해를 구할 수 있습니다.

1. 해가 없음
2. 단 하나의 해
3. 무한히 많은 해

선형 회귀(linear regression)은 Example 2.1과 같은 예제에서 선형 방정식을 풀지 못했을 경우 이를 푸는 방법입니다.

### 선형 방정식에 대한 기하학적 해석(Geometric Interpretation of System of Linear Equation)

두 개의 변수($$x_{1}, x_{2}$$)를 갖는 선형 방정식에서 각각의 방정식은 $$x_{1}$$ - $$x_{2}$$  평면에서 하나의 선이 됩니다.

만약에 각 방정식을 모두 만족하는 해가 있다면, 이 해는 각 선들의 교집합이 됩니다.
...
이를 확장하여 변수가 세 개가 된다면, 각 방정식들은 면(plane)이 되며, 교집합은 선(line)이 됩니다.
...
행렬 또한 선형 대수에서 중요한 역활을 합니다. 행렬은 선형 방정식을 간단히 표기하는데 사용하기도 하지만, Section 2.7에서 살펴볼 선형 함수(linear mapping)를 표현하기도 합니다.
...
두 행렬 $A \in R^{m \times n}$, $B \in R^{n \times k}$가 있을 때, 프로덕트(product) $C = AB \in R^{m \times k}$의 원소 $c_{ij}$는 다음과 같이 계산합니다
...
$R^{n \times n}$에서 단위 행렬(identity matrix)는 대각방향으로는 1, 그 외에는 0을 갖는 $n \times n$- 행렬로 정의됩니다.
...
지금까지 우리는 행렬의 1) 합, 2) 곱, 3) 단위 행렬에 대해서 정의했다. 이를 이용하여 매트릭스의 속성을 살펴보면 아래와 같습닌다.
...
- Multiplication with the identity matrix (단위 행렬과의 곱셈)
...
모든 행렬 $A$가 역행렬 $A^{-1}$를 갖진 않습니다. $A$가 역행렬을 가지려면 $A$는 regular / invertible / nonsigular(정칙행렬) 이어야합니다. 다른 경우는 singular / noninvertible (특이 행렬)이라고 부릅니다.

역행렬이 존재한다면, 이는 유일합니다. Section 2.3에서는 선형 방정식을 풀어 역행렬을 계산하는 일반적인 방법을 논의합니다.
...
행렬 $$A, B$$가 다음과 같을 때 $$A \in R^{m \times n}$$, $$B \in R^{n \times m}$$, $$b_{ij} = a_{ji}$$면 $$B$$를 $$A$$의 전치행렬(transpose)라고 부릅니다. 전치행렬은 $$B = A^{T}$$로 표기합니다.
...
대칭행렬은 정방행렬인 $(n, n)$-행렬에서만 가능하며, 역행렬을 갖으며, 역행렬은 $A^{T}$입니다.
...
이 때, $\lambda$는 $A$의 원소를 스캐일링한다고 볼 수 있습니다.
...
위와 같은 선형 방정식이 있을 때, 선형 방정식을 하나하나 표기하는 것이 아닌, 계수(coefficient $a_{ij}$)들의 집합과의 관계로 표기하게 되면, 행렬곱으로 간소화하여 표현할 수 있습니다.
...
이때, $$x_{1}$$은 첫번째 column을, $$x_{2}$$는 두번째 column을, $$x_{3}$$은 세번째 column을 스캐일링한다고 볼 수 있습니다.

일반적으로 선형 방정식은 행렬 형태로($$Ax = b$$ ) 간소화하여 표현할 수 있습니다.
