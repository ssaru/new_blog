...
이번 포스팅은 노트북에서 클러스터 환경에서 Ray를 활용해서 병렬,분산 어플리케이션을 만드는 방법에 대해서 설명합니다.
...
파이썬 함수 `f`를 "remote function"(remote function은 원격에서 비동기적으로 실행됩니다)으로 바꾸기 위해서는 함수에 `@ray.remote`라는 데코레이터를 선언해줘야합니다. 그리고 함수를 `f.remote()`로 호출하면 즉시 future를 리턴합니다.(future는 미래에 있을 출력에 대한 참조값이다) 그리고 실제 함수의 실행은 백그라운드에서 실행됩니다(우리는 이러한 실행을 task라고 언급했었습니다).
...
task(백그라운드에서 실행되는 _remote function_)는 또 다른 task에 의존할 수 있습니다. 
